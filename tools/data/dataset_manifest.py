            ...
            for line_idx, line in enumerate(f):
                try:
                    ...
                    if feat.shape != tuple(self.stats["feature_shape"]):
                        self.stats["errors"].append(f"Sample {line_idx} has invalid shape {feat.shape}")
                    labels.append(data["label"])
                except Exception as e:
                    self.stats["errors"].append(f"Line {line_idx} failed parse: {str(e)}")

        self.stats["sample_count"] = len(labels)
        self.stats["class_distribution"] = dict(Counter(labels))
        self.stats["validation_status"] = "pass" if not self.stats["errors"] else "fail"
...
if __name__ == "__main__":
    import sys
    path = sys.argv[1] if len(sys.argv) > 1 else "data/training/synthetic_dataset_v2.jsonl"
