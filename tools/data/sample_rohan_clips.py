...
DEFAULT_REVIEW_CLIP_DURATION_S = 60.0


def get_single_clip(video_path, output_dir, clip_idx, domain="youth", duration=DEFAULT_REVIEW_CLIP_DURATION_S):
    """Extract a single random review segment from a video file."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clip-duration", type=float, default=DEFAULT_REVIEW_CLIP_DURATION_S)
    ...
    while sampled_count < args.max_total:
        ...
        if get_single_clip(video, args.output_dir, sampled_count, duration=args.clip_duration):
            sampled_count += 1
