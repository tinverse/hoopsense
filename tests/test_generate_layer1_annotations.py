...
class TrackRepairTest(unittest.TestCase):
    ...
    def test_repair_short_track_gaps_emits_assignment_hypotheses_for_ambiguous_group(self):
        ...
        self.assertGreaterEqual(summary["ambiguous_track_identity_option_count"], 2)
        self.assertTrue(any(item["track_id"] == 19 for item in summary["track_identity_options"]))
        self.assertEqual(repaired[1]["detections"], [])
        successor = next(d for d in repaired[2]["detections"] if d["track_id"] == 19)
        self.assertEqual(successor["identity_option_track_id"], 19)
        self.assertEqual(successor["identity_option_count"], 2)
        self.assertTrue(successor["identity_is_ambiguous"])
        self.assertEqual(set(successor["identity_option_canonical_track_ids"]), {3, 19})
        self.assertTrue({"g0a0", "g0a1"}.issubset(set(successor["identity_option_group_hypothesis_ids"])))
        self.assertEqual({option["canonical_track_id"] for option in successor["identity_options"]}, {3, 19})
    ...
    def test_repair_short_track_gaps_bridges_short_identity_switch(self):
        ...
        self.assertEqual(inserted[0]["identity_option_track_id"], 3)
        self.assertEqual(inserted[0]["identity_option_count"], 1)
        self.assertFalse(inserted[0]["identity_is_ambiguous"])
        self.assertEqual(inserted[0]["identity_best_canonical_track_id"], 3)
        ...
        self.assertEqual(successor["identity_option_track_id"], 19)
        self.assertEqual(successor["identity_option_count"], 1)
        self.assertFalse(successor["identity_is_ambiguous"])
        self.assertEqual(successor["identity_best_canonical_track_id"], 3)
        self.assertEqual(successor["identity_option_canonical_track_ids"], [3])
