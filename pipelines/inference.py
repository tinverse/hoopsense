        ...
        while cap.isOpened():
            ...
            if results[0].boxes.id is not None:
                ...
                for i, tid in enumerate(tids):
                    if classes[i] == 0:
                        if tid not in tracks: tracks[tid] = TrackManager(tid)
                        tm = tracks[tid]
                        raw_court_pos = project_pixel_to_court(float(boxes[i][0]), float(boxes[i][1]), H)
                ...
                for tid in player_map.keys():
                    ...
                    if tm.is_ready() and brain:
                        feat_tensor = construct_features_v2(tm.kpt_history, last_ball_2d, H, np.array([tm.court_x, tm.court_y]), None, None, device)
                        with torch.no_grad():
                            output = brain(feat_tensor)
                            learned_label = LABEL_MAP_INV[int(torch.argmax(output))]
                    context = {"has_possession": possession_engine.current_handler == tid}
                    tm.state_machine.update(tm.kpt_history, learned_label=learned_label, context=context)
                    f.write(json.dumps({"kind": "player", "track_id": int(tid), "t_ms": t_ms, "action": tm.state_machine.get_label(), "court_x": tm.court_x, "court_y": tm.court_y}) + "\n")
    cap.release()
...
if __name__ == "__main__":
    ...
    extract_game_dna(video_path=args.video_path, output_dir=args.output_dir, smoke_test=args.smoke_test)
